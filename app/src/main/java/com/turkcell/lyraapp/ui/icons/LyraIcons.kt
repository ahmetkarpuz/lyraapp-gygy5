 ...
 * LyraApp ikon seti.
 *
 * Material Icons bagimliligi eklemeden, ekranlarin ihtiyac duydugu glyph'leri
 * 24x24 viewport'lu [ImageVector] olarak tanimlar. Path'in dolgu rengi onemsizdir;
 * `Icon(...)` composable'i `tint` ile uzerine yazar. Bu yuzden tum path'ler
 * [Color.Black] ile doldurulur ve renk daima cagri tarafinda temadan okunur.
 */
object LyraIcons {

    /** Marka logosu: ekolayzer/dalga formu cubuklari (Material GraphicEq). */
    ...
    /** Telefon numarasi alaninin leading ikonu (Material Smartphone, outlined). */
    ...
    /** Sifre alaninin leading ikonu (Material Lock). */
    ...
    /** Sifre gorunurluk (goz) ikonu (Material Visibility). */
    ...
    /** Giris butonu ileri oku (Material ArrowForward). */
    ...
    /** Ust bardaki geri oku (Material ArrowBack). */
    ...
    /** Ana sayfa ust bolumundeki tema (acik mod / gunes) ikonu (Material LightMode). */
    ...
    // -- Alt gezinme cubugu (LyraBottomBar) ikonlari --

    /** Ana sayfa sekmesi, secili durum (Material Home, filled). */
    ...
    /** Ana sayfa sekmesi, secimsiz durum (Material Home, outlined). */
    ...
    /** Ara sekmesi (Material Search; dolu/outlined varyanti aynidir). */
    ...
    /** Kutuphane sekmesi, secili durum (Material LibraryMusic, filled). */
    ...
    /** Kutuphane sekmesi, secimsiz durum (Material LibraryMusic, outlined). */
    ...
    /** Favoriler sekmesi, secili durum (Material Favorite, filled). */
    ...
    /** Favoriler sekmesi, secimsiz durum (Material FavoriteBorder). */
    ...
    /** Profil sekmesi, secili durum (Material Person, filled). */
    ...
    /** Profil sekmesi, secimsiz durum (Material Person, outlined). */
...
/**
 * 24x24 viewport'lu, tek path'li bir [ImageVector] uretir.
 * Path verisi standart SVG/Android `pathData` string formatindadir.
 */
